...
epitab(table(covdata$KBY, covdata$outcome))
...
table(is.na(impdata$Bmi_2))
table(is.na(impdata$Bmi_2), impdata$outcome)
table(is.na(ht$Bmi_2))
table(is.na(ht$Bmi_2), ht$outcome)
table(is.na(m$Bmi_2))
table(m$exposure, is.na(m$Bmi_2))
prism_plot("Bmi_2", "outcome", mydata = impdata)
prism_plot("Bmi_2", "long_hospitalization", mydata = impdata)
prism_plot("Bmi_2", "iCU_yesno", mydata = impdata)
prism_plot("Bmi_2", "Ex", mydata = impdata)
densitycurves("Bmi_2", data=ht, groups = "exposure")
densitycurves("Bmi_2", data=m, groups = "exposure") # If anything, controls had lower BMI
prism_plot("Bmi_2", "exposure", mydata = ht)
print(prism_plot("Bmi_2", "exposure", mydata = m)) # If anything, controls had lower BMI
prism_plot("Bmi_2", "outcome", mydata = m)
kruskal.test(m$Bmi_2, m$exposure) # Non-significance does not really tell much due to low data availability
summary(aov(Bmi_2~exposure, m)) # Non-significance does not really tell much due to low data availability
...
table(m$exposure, m$COPD_asthma_history); fisher.test(table(m$exposure, m$COPD_asthma_history)) # Not perfect
...
table(ht$exposure, ht$KBY)
table(m$exposure, m$KBY) # Looks evenly distributed
...
table(ht$exposure, ht$Loop_diuretic_enc)
table(m$exposure, m$Loop_diuretic_enc)

table(ht$exposure, ht$Alpha1_blocker_enc)
table(m$exposure, m$Alpha1_blocker_enc)
table(m$exposure, m$Alpha1_blocker)
